...
void main( uint3 Gid : SV_GroupID, uint GI : SV_GroupIndex, uint3 GTid : SV_GroupThreadID, uint3 DTid : SV_DispatchThreadID )
{
	...
	depth = max(depth, DepthSource[startST + uint2(1, 0)]);
	depth = max(depth, DepthSource[startST + uint2(0, 1)]);
	depth = max(depth, DepthSource[startST + uint2(1, 1)]);
	...
	if (GTid.y == 0)
	{
		...
		for (uint i = 0; i < 8; i++)
		{
			...
			depth2 = max(depth2, g_Cache1[destIdx1]);
		}
	...
	if (GTid.y == 0 && GTid.x == 0)
	{
		...
		for (uint i = 0; i < 8; i++)
		{
			depthTotal = max(depthTotal, g_Cache2[i]);
		}
