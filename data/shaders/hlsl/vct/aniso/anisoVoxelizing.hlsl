...
float4 PSMain(PS_Input pin) : SV_TARGET
{
	...
	// Copy previous face data to current for temporal stability
	[unroll]
	for (uint f = 0; f < 6; f++)
	{
		Texture3D prevFace = GetTexture3D(GetPrevFaceTexId(VoxelInfo.Voxels[VoxelIdx], f));
		float3 prevUv = posUV - VoxelInfo.Voxels[VoxelIdx].MoveOffset * StepSize;
		float4 prev = prevFace.Load(float4(prevUv, 0));

		RWTexture3D<float4> currentFace = ResourceDescriptorHeap[GetFaceTexId(VoxelInfo.Voxels[VoxelIdx], f)];
		currentFace[posUV] = prev;
	}
