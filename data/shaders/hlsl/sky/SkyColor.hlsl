...
float3 getSkyColor(float3 dir, SkyParams Sky, SamplerState sampler)
{
	...
	float3 sunZenithColor = GetTexture2D(Sky.TexIdSunZenith).Sample(sampler, float2(sunZenithDot01, 0.5)).rgb * 0.5f * Sky.EclipseFactor;

	float3 viewZenithColor = GetTexture2D(Sky.TexIdViewZenith).Sample(sampler, float2(sunZenithDot01, 0.5)).rgb * 0.5f * Sky.EclipseFactor;
...
float3 getSkyFullColor(float3 dir, SkyParams Sky, SamplerState sampler)
{
	...
	float3 sunViewColor = GetTexture2D(Sky.TexIdSunView).Sample(sampler, float2(sunZenithDot01, 0.5)).rgb * Sky.EclipseFactor;
