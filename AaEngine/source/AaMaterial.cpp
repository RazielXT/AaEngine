...
ID3D12PipelineState* MaterialBase::GetPipelineState(const std::vector<D3D12_INPUT_ELEMENT_DESC>& layout)
{
	...
	auto pipelineState = CreatePipelineState(layout);

	pipelineStates.emplace_back(layoutHash, layout, pipelineState);

	return pipelineState;
}

ID3D12PipelineState* MaterialBase::CreatePipelineState(const std::vector<D3D12_INPUT_ELEMENT_DESC>& layout)
{
...
AaMaterial* MaterialBase::GetAssignedMaterial(MaterialInstance* instance, const std::vector<D3D12_INPUT_ELEMENT_DESC>& layout)
{
	auto& mats = assignedMaterials[instance];

	auto pipeline = GetPipelineState(layout);
...
void MaterialBase::ReloadPipeline()
{
	...
	{
		auto newPipeline = CreatePipelineState(s.layout);
...
std::unique_ptr<MaterialInstance> MaterialBase::CreateMaterialInstance(const MaterialRef& childRef)
{
	Load();
	...
	for (const auto& t : childRef.resources.textures)
	{
		if (!t.file.empty())
		{
			auto texture = AaTextureResources::get().loadFile(renderSystem->device, renderSystem->commandQueue, t.file);
			mgr.createShaderResourceView(renderSystem->device, *texture);

			instance->AddTexture(*texture);
		}
		else if (!t.id.empty())
		{
...
std::shared_ptr<ResourcesInfo> MaterialBase::CreateResourcesData(const MaterialRef& childRef) const
{
	...
	{
		ResourcesInfo::Texture r;
		r.rootIndex = rootIndex++;
		resources->textures.push_back(r);
	}
...
AaMaterial* MaterialInstance::Assign(const std::vector<D3D12_INPUT_ELEMENT_DESC>& layout)
{
	return base.GetAssignedMaterial(this, layout);
}
...
void MaterialInstance::AddTexture(ShaderTextureView& texture)
{
...
{
	resources->textures[slot].texture = &texture;
...
{
	UINT texCounter = base.info.textures.size();
	for (auto p : resources->autoParams)
	{
		auto& buff = resources->cbuffers[p.bufferIdx].defaultData;
		if (p.type == ResourcesInfo::AutoParam::TEXID && resources->textures.size() > texCounter)
			*(UINT*)&buff[p.bufferOffset] = resources->textures[texCounter++].texture->srvHeapIndex;
	}
}
...
// 		else
// 			commandList->SetGraphicsRootConstantBufferView(cb.rootIndex, cb.buffer[frameIndex].resource->GetGPUVirtualAddress());
	}
}
