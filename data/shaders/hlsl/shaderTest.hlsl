...
float getShadow(Texture2D shadowmap, float4 wp)
{
...
float4 PSMain(PSInput input) : SV_TARGET
{
	...
	float diffuse = saturate(dot(-SunDirection, normal));
    ...
    return (input.color + getShadow(GetTexture(TexIdShadow), input.worldPosition) * diffuse) * GetTexture(TexIdColor).Sample(g_sampler, input.uv) * MaterialColor;
}
