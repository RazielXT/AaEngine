...
#include "FrameCompositor/Tasks/SsaoComputeTask.h"
...
void FrameCompositor::reloadPasses()
{
	...
	for (auto& p : info.passes)
	{
		...
		if (!task)
		{
			...
			{
				pass.task = std::make_shared<SceneRenderTask>(provider, renderWorld, shadowMaps);
			}
			...
			else if (pass.info.task == "DeferredVRT")
			{
				pass.task = std::make_shared<DeferredVrtComputeTask>(provider, renderWorld);
			}
