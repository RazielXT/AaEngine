...
public:
	...
	void BuildLOD(const Camera& camera, XMINT2 offset)
	{
		...
		auto worldSize = m_smallestTileSize * TilesWidth;
		Vector3 chunkOffset = Vector3(offset.x * worldSize, 0, offset.y * worldSize);
		Vector3 cameraPosCentered = cameraPos - chunkOffset;
		// Iterate through all Root Blocks
		for (uint32_t bx = 0; bx < numBlocksX; ++bx) {
			for (uint32_t by = 0; by < numBlocksY; ++by) {
				// Each root is a TilesWidth x TilesWidth unit block
				RecursiveSubdivide(bx * TilesWidth, by * TilesWidth, TilesWidth, 0, cameraPosCentered, chunkOffset, frustum);
			}
		}
	}
...
private:
	...
	void RecursiveSubdivide(uint32_t x, uint32_t y, uint32_t size, uint32_t level, const Vector3& cameraPos, const Vector3& worldOffset, const BoundingFrustum& frustum)
	{
		// Center calculation in world space
		float worldHalfSize = (size * 0.5f) * m_smallestTileSize;
		Vector3 tileCenterWorld = m_gridOrigin + Vector3(
		...
		if (level < (lodsLevels - 1) && distSq < m_subdivideThresholdsSq[level]) {
			...
			RecursiveSubdivide(x, y, child, next, cameraPos, worldOffset, frustum);
			RecursiveSubdivide(x + child, y, child, next, cameraPos, worldOffset, frustum);
			RecursiveSubdivide(x, y + child, child, next, cameraPos, worldOffset, frustum);
			RecursiveSubdivide(x + child, y + child, child, next, cameraPos, worldOffset, frustum);
		}
		else
		{
			BoundingBox bbox;
			bbox.Center = tileCenterWorld + worldOffset;
