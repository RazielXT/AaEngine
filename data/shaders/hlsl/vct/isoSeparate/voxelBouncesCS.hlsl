...
Texture3D<float> SceneVoxelBouncesOccupancyPrev : register(t2);

RWTexture3D<float3> SceneVoxelBounces : register(u0);
RWTexture3D<float> SceneVoxelBouncesOccupancy : register(u1);
...
void main(uint3 id : SV_DispatchThreadID)
{
	...
	float4 traceSample = ConeTrace(voxelUV, worldNormal, 1.05f, 1.5f, SceneVoxelBouncesPrev, SceneVoxelBouncesOccupancyPrev, VoxelSampler);

	float3 bounceColor = baseColor * traceSample.rgb * 0.25f;
	float shadow = diffuse.w;
	bounceColor += baseColor * shadow * 0.75f;
	...
	SceneVoxelBounces[id] = currentLightBounce;
	SceneVoxelBouncesOccupancy[id] = 1 + fade;
}