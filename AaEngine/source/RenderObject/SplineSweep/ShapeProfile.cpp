...
ShapeProfile2D ShapeProfile2D::createArc(float radius, float angleRadians, size_t segments)
{
	...
	const float startAngle = -Pi * 0.5f - angleRadians * 0.5f;
	std::vector<ShapeProfilePoint> points;
	...
	for (size_t i = 0; i <= segments; ++i)
	{
		...
		const Vector2 radial = { std::cos(angle), std::sin(angle) };
		points.push_back({ radial * radius, -radial });
	}
...
ShapeProfile2D ShapeProfile2D::createFilledArc(float radius, float angleRadians, size_t segments)
{
	ShapeProfile2D profile;
	radius = clampPositive(radius);
	if (segments < 1)
	...
	for (size_t i = 0; i <= segments; ++i)
	{
		const float t = static_cast<float>(i) / static_cast<float>(segments);
		const float angle = startAngle + angleRadians * t;
		positions.push_back({ std::cos(angle) * radius, std::sin(angle) * radius });
	}

	positions.push_back({ 0.0f, 0.0f });
	profile.addContour(positions, true);
	return profile;
}
