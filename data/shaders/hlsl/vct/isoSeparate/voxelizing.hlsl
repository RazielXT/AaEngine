...
float4 PSMain(PS_Input pin) : SV_TARGET
{
	...
	Texture3D<float4> SceneVoxelPrevious = GetTexture3D(VoxelInfo.Voxels[VoxelIdx].TexIdPrev);
	Texture3D<float> SceneVoxelPreviousOccupancy = GetTexture3D1f(VoxelInfo.Voxels[VoxelIdx].TexIdPrevOccupancy);
	float3 prevUv = posUV - VoxelInfo.Voxels[VoxelIdx].MoveOffset * StepSize;
	float3 prevColor = SceneVoxelPrevious.Load(float4(prevUv, 0)).rgb;
	float prevOccupancy = SceneVoxelPreviousOccupancy.Load(float4(prevUv, 0)).r;

	RWTexture3D<float3> SceneVoxel = ResourceDescriptorHeap[VoxelInfo.Voxels[VoxelIdx].TexId];
	RWTexture3D<float> SceneVoxelOccupancy = ResourceDescriptorHeap[VoxelInfo.Voxels[VoxelIdx].TexIdOccupancy];
	SceneVoxel[posUV] = prevColor;
	SceneVoxelOccupancy[posUV] = prevOccupancy;
