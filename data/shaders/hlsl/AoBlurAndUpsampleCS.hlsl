...
{
	Texture2D<float2> LoResAO1 = ResourceDescriptorHeap[ResIdLoResAO1];
    float4 AO1 = LoResAO1.Gather( LinearSampler, uv );

#ifdef COMBINE_LOWER_RESOLUTIONS

	Texture2D<float2> LoResAO2 = ResourceDescriptorHeap[ResIdLoResAO2];
	...
	Texture2D<float2> LoResDB = ResourceDescriptorHeap[ResIdLoResDB];
...
void main( uint3 Gid : SV_GroupID, uint GI : SV_GroupIndex, uint3 GTid : SV_GroupThreadID, uint3 DTid : SV_DispatchThreadID )
{
	...
	Texture2D<float2> HiResAO = ResourceDescriptorHeap[ResIdHiResAO];
	...
	Texture2D<float2> HiResDB = ResourceDescriptorHeap[ResIdHiResDB];
    float4 HiDepths = HiResDB.Gather(LinearSampler, UV1);

    int2 OutST = DTid.xy << 1;
	RWTexture2D<float2> AoResult = ResourceDescriptorHeap[ResIdAoResult];
